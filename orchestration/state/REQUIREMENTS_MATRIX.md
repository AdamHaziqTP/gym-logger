...
| REQ-024 | Never claim rich Notes structure/color works without target-iPhone paste evidence. | Spec §§15.4-15.6, 32 | M03 | PARTIALLY VERIFIED (captured native replay and Phase A removal PASS; Phase A2 single-type sufficiency pending; generated Gym payload pending) | M03-T06 exact HTML replay: editable table/data/order PASS, colours FAIL; E-004 exact captured Notes replay and every Phase A removal variant: editable table/data/order/Unicode/colours PASS; A2 single-representation variants are implemented but await target-iPhone results; generated Gym Logger native payload remains unproven |
