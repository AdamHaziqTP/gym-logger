...
- Human review gate: install the single `GymLogger-unsigned.ipa` artifact from hosted run `32990579200` and complete `orchestration/evidence/HUMAN-VERIFICATION-M03-T11-FINAL-IPHONE14-PROMAX.md` on the iPhone 14 Pro Max. Desktop/source evidence must not close this gate.
...
- Hosted run `32990405957` exposed an Xcode 15.4 Swift API rename in the new Photos asset creation call. Codex corrected it to `PHAssetCreationRequest.forAsset()` and reran local tests/build/static/native checks successfully; replacement run `32990579200` then passed.
- M03-T11 hosted checkpoint: run `32990579200` passed macOS/Xcode compilation, unsigned IPA packaging, artifact assertions, and upload. Codex independently inspected the artifact's bundle identity/executable, Photos add-only usage description, bundled web app, icons, and compiled native action. The consolidated iPhone 14 Pro Max gate is open.
...
- M03-T11 final IPA UX cleanup is implemented at `fbcb062`: only the coloured Notes action and direct Photos snapshot action remain in the session UI; the native bridge uses add-only Photos authorization and reports truthful completion states. Codex local verification is 410/410 tests, build PASS, 27/27 one-app checks, native checks PASS, and diff hygiene PASS. Hosted run `32990579200` is independently packaged and inspected; only the consolidated iPhone gate remains.
