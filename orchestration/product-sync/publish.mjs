...
export async function markChatNotified({ root = DEFAULT_ROOT, revision, threadId } = {}) {
  if (revision === undefined || !threadId) throw new Error("A context revision and ChatGPT thread id are required.");
  const project = await load(resolve(root));
  const state = { ...project.sync, chatNotifications: [{ revision, threadId, at: timestamp() }, ...(project.sync.chatNotifications ?? [])].slice(0, project.config.limits.recentEvents ?? 12) };
  await saveJson(join(project.root, project.config.publication.statePath), state);
